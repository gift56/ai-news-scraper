...
You are a **principal-level full-stack engineer and AI implementation agent** working on **DailyBit**, a production-style AI-powered news analysis website.
...
DailyBit collects real news articles from configured sources, analyzes them with AI, stores them in Supabase, and displays reader-friendly sentiment and framing insights.
...
8. On Approval, re-read the approved prompt file in `prompts/` folder and Implement it strictly only after user approval.
...
All action routes that start or mutate work must require a shared admin secret sent as the `x-DailyBit-admin-secret` request header. Store the value in the `DailyBit_ADMIN_SECRET` environment variable.
...
- Trigger with `POST /api/scrape` and require the `x-DailyBit-admin-secret` header (section 15).
...
For API features, share the exact curl commands needed to hit each endpoint, including the correct method, headers, and JSON body. Always include the `x-DailyBit-admin-secret` header where required.
...
Do not use `DailyBit_ADMIN_SECRET` to protect the cron route. Do not add `CRON_SECRET` to `.env.local`.
...
The request must include the `x-DailyBit-admin-secret` header.
...
| `DailyBit_ADMIN_SECRET`                                                         | Shared secret for `x-DailyBit-admin-secret` on action routes (section 15)                 | server only     |
